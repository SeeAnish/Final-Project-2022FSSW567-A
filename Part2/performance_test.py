...
def speed_test_decode_with_test( first , last , fileName , step_num):
    with open('records_decoded.json') as json_file:
        tic = time.perf_counter()
        data = json.load(json_file)
...
def speed_test_decode_without_test( first , last , fileName , step_num):
    with open('records_decoded.json') as json_file:
        tic = time.perf_counter()
        data = json.load(json_file)
        records = data["records_decoded"]
...
if __name__ == '__main__':
    ...
    speed_test_decode_with_test(0 , 10000 , "decode with test.csv" , 1)
    speed_test_decode_without_test(0, 10000, "decode without test.csv", 1)